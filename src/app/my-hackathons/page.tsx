      ...
      <main className="flex-1 p-6 md:p-8 space-y-6 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {MOCK_HACKATHONS.map((h) => (
            <div key={h.id} className="bg-[#FFFFFF] p-7 space-y-5 rounded-3xl shadow-xs hover:shadow-md transition-all">
              <div className="flex justify-between items-start">
                <div className="space-y-1">
                  <h2 className="text-xl font-geist font-bold text-[#111111]">{h.title}</h2>
                  <p className="text-xs text-[#777777] font-inter">{h.tagline}</p>
                </div>
              </div>

              <div className="pt-2 flex items-center justify-between font-inter text-xs">
                ...
                <Link
                  ...
                  className="px-5 py-2.5 bg-[#800000] hover:bg-[#660000] text-white text-xs font-inter font-bold rounded-full transition-colors flex items-center gap-2 shadow-xs"
                >
