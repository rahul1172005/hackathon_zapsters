    ...
    <div className="min-h-screen bg-[#F7F7F5] dark:bg-black text-[#111111] dark:text-white flex flex-col font-inter transition-colors duration-200">
      ...
      <section className="bg-[#FFFFFF] dark:bg-[#141414] py-12 shadow-xs border-b border-transparent dark:border-neutral-800">
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex items-start gap-5">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-[#777777] dark:text-neutral-400 max-w-2xl leading-normal mt-2 font-inter">{p.bio}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 font-inter text-xs">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center gap-2 px-5 py-2.5 bg-[#F7F7F5] dark:bg-neutral-900 hover:bg-[#E5E5E2] dark:hover:bg-neutral-800 text-[#111111] dark:text-white border border-[#E5E5E2] dark:border-neutral-800 rounded-full font-bold transition-colors"
              >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 pt-4 font-inter">
            <div className="p-5 bg-[#F7F7F5] dark:bg-neutral-900 border border-transparent dark:border-neutral-800/80 rounded-2xl">
              <div className="text-xs text-[#777777] dark:text-neutral-400 uppercase font-bold">HACKATHONS</div>
              <div className="text-3xl font-bold font-geist text-[#111111] dark:text-white mt-1">{p.stats.hackathonsCount}</div>
            </div>
            <div className="p-5 bg-[#F7F7F5] dark:bg-neutral-900 border border-transparent dark:border-neutral-800/80 rounded-2xl">
              <div className="text-xs text-[#777777] dark:text-neutral-400 uppercase font-bold">GRAND WINS</div>
              <div className="text-3xl font-bold font-geist text-[#800000] dark:text-red-400 mt-1">{p.stats.wins}</div>
            </div>
            <div className="p-5 bg-[#F7F7F5] dark:bg-neutral-900 border border-transparent dark:border-neutral-800/80 rounded-2xl">
              <div className="text-xs text-[#777777] dark:text-neutral-400 uppercase font-bold">FINALS</div>
              <div className="text-3xl font-bold font-geist text-[#111111] dark:text-white mt-1">{p.stats.finals}</div>
            </div>
            <div className="p-5 bg-[#F7F7F5] dark:bg-neutral-900 border border-transparent dark:border-neutral-800/80 rounded-2xl">
              <div className="text-xs text-[#777777] dark:text-neutral-400 uppercase font-bold">PROJECTS</div>
              <div className="text-3xl font-bold font-geist text-[#111111] dark:text-white mt-1">{p.stats.projectsCount}</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 w-full flex-1 space-y-8">
        <div className="space-y-6">
          <h2 className="text-2xl font-geist font-bold text-[#111111] dark:text-white">Featured Shipped Projects</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {p.projects.map((proj, idx) => (
              <div key={idx} className="bg-[#FFFFFF] dark:bg-[#141414] border border-[#E5E5E2] dark:border-neutral-800 p-7 space-y-4 rounded-3xl hover:border-[#800000] transition-colors shadow-xs">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="text-xl font-geist font-bold text-[#111111] dark:text-white">{proj.name}</h3>
                    <div className="text-xs font-inter text-[#777777] dark:text-neutral-400 mt-1">{proj.hackathonName} • {proj.year}</div>
                  </div>
                ...
                <p className="text-sm text-[#777777] dark:text-neutral-400 leading-relaxed font-inter">{proj.description}</p>
              </div>
            ))}
          </div>
        </div>
